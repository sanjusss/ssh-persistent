---
name: ssh-persistent
description: 通过 SSH 长连接远程排查服务器。支持直连/标准跳板(`ControlMaster` 多路复用)和堡垒机(仅交互终端)两种场景,用 `ssh-mux.py` 统一管理连接、命令执行和文件传输。当需要对远程服务器执行多次 SSH 命令时使用。
---
